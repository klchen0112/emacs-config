...
let
  makeProfile =
    ...
    lib.nameValuePair
      "${presets.name}-${otherSettings.name}${if personalized then "-personalized" else ""}"
      (
        emacs-config.override (
          _:
          {
  ...
  go =
    ...
    lib.flatten (builtins.map (prev: builtins.map (value: prev // { ${key} = value; }) values) acc);
in
lib.pipe matrix [
